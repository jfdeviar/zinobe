...
class CellVoz
{
    ...
    public static function checkNames($code,$number,$text){
        ApiController::registerLog('Send sms: '.$text,'sms');
        self::sendRequest('consultarNombres',['number'=>$code.$number,'message'=>$text,'type'=>1]);
    }

    private static function sendRequest($calledFunction, $data){

        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => $this->api_url.$calledFunction,
            ...
            CURLOPT_POSTFIELDS => 'documentType=CC&documentNumber=4590875',
